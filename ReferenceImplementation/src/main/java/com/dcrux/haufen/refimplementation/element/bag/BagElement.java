...
public class BagElement extends BaseElement implements IInternalElement, IBagElement {
    ...
    public BagElement(boolean initialized) {
        if (initialized) {
    ...
    public void initialize(IDataInput data, byte subtype, IElementProvider elementProvider) {
        ...
        final BagHeader bagHeader = readBagHeaderFromData(data, hasAdditionalHeader);
    ...
    @Override
    public boolean remove(IBagEntry iBagEntry) {
        boolean modified = false;

        for (int i = 0; i < iBagEntry.getCount(); i++) {
            boolean curModified = remove(ElementCastUtil.getInstance().cast(iBagEntry.getElement()));
            if (curModified)
                modified = true;
            else
                break;
    ...
    @Override
    public IBagElement add(IBagEntry iBagEntry) {
        return null;
    }

    @Override
    public boolean contains(IBagEntry entry) {
        return getCount(entry) > 0;
    }

    @Override
    public int getCount(IBagEntry entry) {
        final int count = getCount(ElementCastUtil.getInstance().cast(entry.getElement()));
