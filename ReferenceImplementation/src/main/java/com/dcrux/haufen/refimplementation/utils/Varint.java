...
public class Varint {
     ...
     * {@link #writeUnsignedVarLong(long, IDataOutput)} should be used.
     *
     ...
     * If values can be negative, use {@link #writeSignedVarLong(long, IDataOutput)}
     ...
     */
    public static void writeSignedVarInt(int value, IDataOutput out) throws IOException {
        // Great trick from http://code.google.com/apis/protocolbuffers/docs/encoding.html#types
     ...
     * @throws IOException              if {@link IDataInput} throws {@link IOException}
     * @see #readSignedVarLong(IDataInput)
     */
    public static int readSignedVarInt(IDataInput in) throws IOException {
    ...
    public static int readUnsignedVarIntCount(byte[] bytes) {
        int i = 0;
        int count = 0;
        for (byte b : bytes) {
            count++;
